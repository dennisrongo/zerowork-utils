...
**[templates/linkedin_feed.json]**. Auto-align **does exist** (React Flow
controls); prefer it over guessing canvas coordinates. Session-only
...
5. **Configure drawers** — no REST write. On the **paired** Chrome window
   ...
   successfully". A click that only shows `ID <n>` did **not**
   open the drawer; do not retry that forever
   ([creator-editor-automation.md](references/creator-editor-automation.md)). Monaco ignores UIA `set_value` — type_text recipe in
   ...
   "Authoring for a human") — do not type it in. **Rename:** dblclick
   the title, **Ctrl+A**, type the job name, Enter (without Ctrl+A the
   new text appends; `PATCH /connector` is the TaskBot name only). Add
   `sticky_note`s for login / selectors / stop conditions. Clear stray
   `x` helpers. Auto-align, then connect remaining edges (REST
   preferred).
