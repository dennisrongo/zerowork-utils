...
- **Drawer config saves over WEBSOCKET, not REST** — a fetch hook catches nothing on SAVE. Drawer-driven config is the only write path; REST covers nodes/edges/tables/columns and the **TaskBot** name.
- Renames: `PATCH /connector/<id>/ {name}` renames the **TaskBot**.
  Node titles have no REST write — dblclick the title Text, **Ctrl+A**,
  type, Enter ([creator-editor-automation.md](creator-editor-automation.md)
  "Rename nodes"). Without Ctrl+A the new text appends.
